                                            ...
                                            <?php if ($indikator['id'] == '60'): ?>
                                                ...
                                                <?php else: ?>
                                                    ...
                                                    $file_bukti = $koneksi->query(
                                                        "SELECT * FROM sub_verifikasi_lapangan_bukti WHERE id_sub_verifikasi_lapangan = " . $adm_tak_bernilai['id'] . ";"
                                                    );
                                                    $show_bukti = '';
                                                    if ($file_bukti->num_rows == 0) {
                                                        $show_bukti = $show_bukti . '<td style="width: 10%;"></td>';
                                                    } else {
                                                        $show_bukti = $show_bukti . '<td style="width: 10%;">Bukti : <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#carouselModal' . $adm_tak_bernilai['id'] . '"><i class="fas fa-eye"></i></button></td>
                                                        <div class="modal fade" id="carouselModal' . $adm_tak_bernilai['id'] . '" tabindex="-1" role="dialog" aria-labelledby="carouselModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog modal-lg" role="document">
                                                        <div class="modal-content">
                                                        <div class="modal-header">
                                                        <h5 class="modal-title" id="carouselModalLabel">Bukti' . $adm_tak_bernilai['id'] . '</h5>
                                                        </div>
                                                        <div class="modal-body">
                                                        <!-- Carousel -->
                                                        <div id="carouselExampleControls' . $adm_tak_bernilai['id'] . '" class="carousel slide" data-ride="carousel">
                                                        <div class="carousel-inner">';

                                                        foreach ($file_bukti as $index => $fileBukti) {
                                                            $carouselItemClass = ($index == 0) ? 'carousel-item active' : 'carousel-item';
                                                            if (pathinfo($fileBukti['file_bukti']) == 'pdf') {
                                                                $show_bukti = $show_bukti . '<div class="' . $carouselItemClass . '">
                                                        ...
                                                        $show_bukti = $show_bukti . '</div>
                                                        <a class="carousel-control-prev" href="#carouselExampleControls' . $adm_tak_bernilai['id'] . '" role="button" data-bs-slide="prev">
                                                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                                        <span class="sr-only">Previous</span>
                                                        </a>
                                                        <a class="carousel-control-next" href="#carouselExampleControls' . $adm_tak_bernilai['id'] . '" role="button" data-bs-slide="next">
                                                                        ...
                                                                        foreach ($adms as $adm) {
                                                                            if ($adm['id_nilai_sub_indikator'] == $nilai_sub_indikator['id']) {
                                                                                ...
                                                                                if ($file_bukti->num_rows == 0) {
                                                                                    ...
                                                                                    $show_bukti = $show_bukti . '<td style="width: 10%;">Bukti : <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#carouselModal' . $adm['id'] . '"><i class="fas fa-eye"></i></button></td>
                                                                                    <div class="modal fade" id="carouselModal' . $adm['id'] . '" tabindex="-1" role="dialog" aria-labelledby="carouselModalLabel" aria-hidden="true">
                                                                                    <div class="modal-dialog modal-lg" role="document">
                                                                                    <div class="modal-content">
                                                                                    <div class="modal-header">
                                                                                    <h5 class="modal-title" id="carouselModalLabel">Bukti' . $adm['id'] . '</h5>
                                                                                    </div>
                                                                                    <div class="modal-body">
                                                                                    <!-- Carousel -->
                                                                                    <div id="carouselExampleControls' . $adm['id'] . '" class="carousel slide" data-ride="carousel">
                                                                                    <div class="carousel-inner">';
                                                                                    ...
                                                                                    $show_bukti = $show_bukti . '</div>
                                                                                    <a class="carousel-control-prev" href="#carouselExampleControls' . $adm['id'] . '" role="button" data-bs-slide="prev">
                                                                                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                                                                    <span class="sr-only">Previous</span>
                                                                                    </a>
                                                                                    <a class="carousel-control-next" href="#carouselExampleControls' . $adm['id'] . '" role="button" data-bs-slide="next">
                                                                                    <span class="carousel-control-next-icon" aria-hidden="true"></span>
                                                                                    <span class="sr-only">Next</span>
                                                                                    </a>
                                                                                    </div>
